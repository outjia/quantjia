...
def parse_params(mstr):
    ...
    models = {'MC':'nbuild_model','MC2':'nbuild_model2'}
    params = {}
    params['mclass'] = ''
...
def predict_today_cnn(mstr, run):
    ...
    if params['model_name'].startswith('MC'):
        dataset = ncreate_today_dataset_cnn(index=index, days=params['lookback'])
    else:
        dataset = ncreate_today_dataset_rnn(index=index, days=params['lookback'])
...
def start_service():
    ...
    s = SimpleXMLRPCServer(('127.0.0.1',8081),allow_none=True)
    s.register_function(predict_today_cnn)
    s.serve_forever()
    # M1T5C3_D1()
    # predict_today()
