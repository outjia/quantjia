...
def train_model_simple(mstr):
    params = parse_params(mstr)
    dataset = dmr.create_trainset(params['stocks'], params['lookback'])
    train, test = dmr.split_dataset(dataset, 0.75, params['batch_size'])
    bstrain, tstrain, lbtrain_v = dmr.create_feedtrain_hp(train)
    bstest, tstest, lbtest_v = dmr.create_feedtrain_hp(test)
    ...
    if not os.path.exists(model_dir): os.makedirs(model_dir)
    callbacks = [
        EarlyStopping(monitor='val_loss', patience=6, verbose=0, mode='max'),
...
def predict_today(mstr):
    # return [confidence, stock_code, current_price]
    params = parse_params(mstr)
    ...
    if 20 > datetime.datetime.now().hour > 10 and datetime.datetime.now().weekday() < 5:
        data_x = tsdata_f[:, -params['lookback']:,1:]
    else:
        data_x = tsdata[:, -params['lookback']:, 1:]
    data_v = rtdata_v
    ...
    data_v = data_v[:len(data_v) / batch_size * batch_size]
...
def validate_m():
    validate_model('M1_T5_B256_C3_E100_S2000')

def P_M1_T5_B256_C3_E100_S100():
    predict_today_simple('M1_T5_B256_C4_E100_S100')
